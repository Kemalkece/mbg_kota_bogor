...
if (carousel) {
    ...
    // Ambil data dari atribut data-* di slide (bukan hardcoded)
    const newsData = Array.from(slides).map(slide => ({
    ...
    function updateCarousel() {
        ...
        if (newsTitle && newsData[currentSlide]) {
            ...
            setTimeout(() => {
                const current = newsData[currentSlide];
                newsTitle.innerText = current.title;
                newsDesc.innerText = current.desc;
                newsLink.setAttribute('href', current.url);
    ...
    function startAutoSlide() {
        autoSlideInterval = setInterval(() => moveCarousel(1), 8000);
    }
    ...
    // Expose ke global agar tombol arrow onclick bisa manggil
...
if (faqBtn) {
    faqBtn.onclick = function () {
        window.location.href = "faq-detail.html";
    };
}